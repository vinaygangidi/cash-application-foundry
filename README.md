...
An experienced AR analyst handles maybe 8–10 of these edge cases per hour. Our system handles 35 of them in under 60 seconds — and shows its reasoning for every single one.
...
### The framework: custom sequential swarm on Azure AI Foundry
...
This is a **hand-off pattern** (also called a pipeline swarm), not a parallel swarm. Here's why that fits cash application specifically:
...
- The sequential pattern makes the audit trail clean: Agent 1 output → Agent 2 output → Agent 3 uses both → etc. Every step is traceable.
...
|---|---|
| **Amount mismatches** (10) | Exact match, multi-invoice bundle, early-pay discount (valid), unauthorized short pay, freight deduction, damage claim, overpayment to credit, credit memo netting, wire fee write-off (≤$25 auto), late discount |
| **Identity & name issues** (4) | SWIFT 35-char truncation, DBA/trade name, post-acquisition name change, fuzzy alias matching |
...
| **Timing problems** (6) | Duplicate payment, installment/partial, NSF return + reversal, post-dated check hold, stale check (>180 days), prepayment to unearned revenue |
...
The current build is optimized for the hackathon — reliable demo, real Azure inference, real streaming. Here's the gap between what we built and what a production deployment looks like:
